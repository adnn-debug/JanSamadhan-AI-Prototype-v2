...
(function(){
  ...
  var DISTRICT_CENTERS={
    ...
    "Chatra":[24.2065,84.8713],
    "Deoghar":[24.4852,86.6948],
    "Dhanbad":[23.7957,86.4304],
    "Dumka":[24.2681,87.2486],
    "East Singhbhum":[22.8046,86.2029],
    "Garhwa":[24.1600,83.8070],
    "Giridih":[24.1856,86.2996],
    "Godda":[24.8270,87.2120],
    "Gumla":[23.0441,84.5370],
    "Hazaribagh":[23.9966,85.3691],
    "Jamtara":[23.9638,86.8028],
    "Khunti":[23.0760,85.2780],
    "Koderma":[24.4675,85.5934],
    "Latehar":[23.7442,84.4990],
    "Lohardaga":[23.4330,84.6799],
    "Pakur":[24.6337,87.8494],
    "Palamu":[24.0398,84.0907],
    "Ramgarh":[23.6341,85.5211],
    "Ranchi":[23.3441,85.3096],
    "Sahibganj":[25.2445,87.6348],
  ...
  function distanceKm(a,b){
    ...
    var R=6371,dLat=(lat2-lat1)*Math.PI/180,dLon=(lng2-lng1)*Math.PI/180;
  ...
  function pendingGeo(){
    try{
      if(window.JanSamadhanGIS&&typeof window.JanSamadhanGIS.getPendingGeo==="function")return window.JanSamadhanGIS.getPendingGeo();
    }catch(e){}
  ...
  function errorText(reason){
    if(isHindi()){
      if(reason==="missing")return"झारखंड क्षेत्र सत्यापित करने के लिए मानचित्र पर स्थान पिन चुनना आवश्यक है।";
    ...
    if(reason==="missing")return"Select a map pin so the system can verify that this report is inside Jharkhand.";
  ...
  function setError(text){
    var e=id("reportError");
    if(e){e.textContent=text;e.setAttribute("role","alert")}
    var s=id("jsjh-guard-status");
    if(s){s.textContent=text;s.className="jsjh-guard-status bad"}
  }

  function setGood(text){
    var s=id("jsjh-guard-status");
    if(s){s.textContent=text;s.className="jsjh-guard-status good"}
  }
  ...
  function syncDistricts(){
    ...
    var html='<option value="">'+(isHindi()?"झारखंड जिला चुनें":"Select Jharkhand district")+'</option>';
    ...
    if(select.dataset.jsjhDistricts!=="1"||select.options.length!==DISTRICTS.length+1){
      ...
      if(DISTRICTS.indexOf(current)>=0)select.value=current;
      else select.value="";
      select.dataset.jsjhDistricts="1";
    }
    var label=document.querySelector('label[for="rDistrict"]');
    if(label)label.textContent=isHindi()?"जिला (केवल झारखंड)":"District (Jharkhand only)";
  }
  ...
  function ensureGuardUI(){
    ...
    if(label)label.textContent=isHindi()?"झारखंड में मानचित्र स्थान (आवश्यक)":"Map location in Jharkhand (required)";
    var status=id("jsgeo-status");
    if(status&&(!pendingGeo()))status.textContent=isHindi()?"रिपोर्ट जमा करने से पहले झारखंड के अंदर स्थान पिन चुनें।":"Choose a pin inside Jharkhand before submitting the report.";
    if(!id("jsjh-jurisdiction-note")){
      var note=document.createElement("div");
      note.id="jsjh-jurisdiction-note";
      note.className="jsjh-note";
      note.innerHTML='<strong>'+(isHindi()?"क्षेत्र सीमा: झारखंड":"Jurisdiction: Jharkhand")+'</strong> '+(isHindi()?"बाहर के पिन या गलत जिला-पिन संयोजन अस्वीकार किए जाएंगे।":"Out-of-state pins and district/pin mismatches are blocked before submission.");
      var map=id("jsgeo-report-map");
      if(map&&map.parentNode)map.parentNode.insertBefore(note,map);
    }
    if(!id("jsjh-guard-status")){
      var guard=document.createElement("div");
      guard.id="jsjh-guard-status";
      guard.className="jsjh-guard-status";
      var intel=id("jsgeo-intel");
      if(intel&&intel.parentNode)intel.parentNode.insertBefore(guard,intel.nextSibling);
      else wrap.appendChild(guard);
    }
  }
  ...
  function validateCurrentPin(options){
    options=options||{};
    var district=id("rDistrict")?String(id("rDistrict").value||""):"";
    var result=geoResult(pendingGeo(),district);
    var map=id("jsgeo-report-map");
    if(result.ok){
      if(map)map.classList.remove("jsjh-invalid-map");
      setGood(isHindi()?"झारखंड स्थान सत्यापित · चयनित जिला: "+district:"Jharkhand location verified · selected district: "+district);
      return true;
    }
  ...
  function bindFormGuard(){
    ...
    if(!form||form.dataset.jsjhGuard==="1")return;
  ...
  function bindLiveValidation(){
    ...
    if(district&&district.dataset.jsjhBound!=="1"){
      ...
      district.addEventListener("change",function(){
        setTimeout(function(){validateCurrentPin({showError:!!pendingGeo(),clearInvalid:true})},0);
      });
    }
    var map=id("jsgeo-report-map");
  ...
  function migrateLegacyDemoData(){
    ...
    store.data.problems.forEach(function(p){
      ...
      if(Array.isArray(p.universities))p.universities.forEach(function(u){
        if(u.provider==="Patna University Community Lab")u.provider="BIT Mesra Innovation Hub";
        if(u.provider==="NIT Patna Technical Outreach")u.provider="NIT Jamshedpur Technical Outreach";
      });
      changed.push(p);
    });
    ...
    cloudTimer=setTimeout(function(){
      try{
        ...
        changed.forEach(function(p){
          db.collection("problems").doc(p.id).set({district:p.district,location:p.location,title:p.title,assignedUniversity:p.assignedUniversity,universities:p.universities},{merge:true}).catch(function(){});
        });
      }catch(e){}
    },1200);
  }
  ...
  function syncStageInstitutions(){
    var select=id("stageUniversity");
    if(!select||select.dataset.jsjhInstitutions==="1")return;
  ...
  function init(){
    ...
    window.JanSamadhanJurisdiction={
      state:"Jharkhand",
      districts:DISTRICTS.slice(),
      validate:function(geo,district){return geoResult(geo,district)},
      version:"1.0"
    };
  }
